...
The top left is the data viewer and/or the source document. It can be used to view data or to edit the r markdown. 
Bottom left is the console. It can be used to do arithmetic, load packages, object assignment, access variables, and use functions. 
Top right is the environment. It shows the datasets and the data within them. The top right can also be used to access the git panel, which can be used to push the project to github. 
Bottom left is the viewer pane. It displays what the code in the r markdown does. 
...
"NA" is used as a placeholder. When a value is missing, it is filled by NA.
...
ggplot(data = plastic_waste, mapping = aes(x = plastic_waste_per_cap)) +
  ...
  labs(x = "Platic waste per capita", y = "Frequency")
```
...
Color and fill are using variables so they are grouped with the data. Alpha is just using a number, though, so it is a characteristic of the graph. 
...
The violin parts show the density of the continents better. The wider the shape of the "violin", the greater frequency it has. The side-by-side box plots do not show this with the accuracy that violin plots do. However, they can display the outliers on the graph, shown by the points. The violin plot does not do this, 
...
ggplot(plastic_waste, aes(x = continent, y = plastic_waste_per_cap)) +
  geom_violin() +
  labs(x = "Continent", y = "Plastic waste per capita")
```
...
```{r}
# same graph as before but colored by continent
ggplot(plastic_waste, aes(x = plastic_waste_per_cap, y = mismanaged_plastic_waste_per_cap, color = continent)) +
  geom_point() +
  labs(x = "Plastic waste per capita", y = "Mismanaged plastic waste per capita")

```
All the continents are generally grouped up towards the bottom left (and a little bit up) of the graph. This means that they dont have much plastic waste per capita but African, Oceania, and Asia have some outliers with more mismanaged waste per capita than other continents. It appears that North America has the most outliers, meaning the countries within that continent are more "all over the place" compared to the rest. 
...
ggplot(plastic_waste, aes(x = plastic_waste_per_cap, y = total_pop)) +
  ...
  labs(title = "Plastic waste per capita vs. Total population", x = "Plastic waste per capita", y = "Total population")
```

```{r}
ggplot(plastic_waste, aes(x = plastic_waste_per_cap, y = coastal_pop)) +
  geom_point(alpha = 0.25) +
  labs(title = "Plastic waste per capita vs. Coastal population", x = "Plastic waste per capita", y = "Coastal population")
```
They do not seem to have much linear association. Looking at the two graphs, it appears that population has little effect on plastic waste per capita, for both total population and coastal population. 
...
The y axis is displaying the plastic waster per capita of all the countries EXCEPT for those that have a per-capita larger than 3. The x axis took the coastal population and divided it by the total population to display the coast population proportion of each country. The graph is color coordinated to show what continent each country (point) belongs to. The line going through the graph shows the overall pattern that all the countries are following when it comes to population and waste per capita. 
